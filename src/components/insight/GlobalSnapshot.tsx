  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
      {globalSnapshots.map((r) => {
        const TrendIcon =
          r.trend === "up" ? ArrowUpRight : r.trend === "down" ? ArrowDownRight : Minus;
        const color =
          r.trend === "up"
            ? "text-positive"
            : r.trend === "down"
        ...
        return (
          <div
            key={r.region}
            className="rounded-xl glass-card p-3.5 hover:border-primary/30 transition"
          >
            ...
            <p className="mt-2.5 text-xs leading-snug text-foreground/90 line-clamp-3 min-h-[3rem]">
              {r.headline}
            </p>
