...
export const Route = createFileRoute("/search")({
  head: () => ({ meta: [{ title: "Search — Insight" }, { name: "description", content: "Search across global intelligence." }] }),
...
function SearchPage() {
  ...
  const results = useMemo(() => {
    ...
    if (query) {
      ...
      r = r.filter((f) => f.headline.toLowerCase().includes(q) || f.summary.toLowerCase().includes(q) || f.category.toLowerCase().includes(q));
    }
  ...
  return (
    <PageShell eyebrow="Search" title="Global search" description="Query the entire intelligence corpus. Filter by time, region, and domain.">
      ...
      <div className="mt-5 space-y-3">
        {filterGroups.map((g) => (
          <div key={g.name} className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground w-16">{g.name}</span>
            {g.options.map((opt) => (
              <button
                key={opt}
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Ranked by relevance</span>
        </div>
        {results.length === 0 ? (
          <div className="rounded-xl glass-card p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">Try a different query or relax your filters.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {results.map((f) => <FeedCard key={f.id} item={f} />)}
          </div>
        )}
      </div>
    </PageShell>
