...
export const Route = createFileRoute("/breaking")({
  head: () => ({ meta: [{ title: "Breaking News — Insight" }, { name: "description", content: "Live breaking news as it happens." }] }),
...
function BreakingPage() {
    ...
    async function loadData() {
      try {
        ...
        if (articles && articles.length > 0) {
          ...
          const critical = mapped.filter((f) => f.importance === "critical" || f.importance === "high");
  ...
  return (
    <PageShell eyebrow="Live" title="Breaking news" description="The most urgent signals in the last 24 hours, ranked by importance.">
      <BreakingBanner />
